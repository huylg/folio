...
final class ColumnMenuTests: XCTestCase {
    ...
    func testTheSubmenuHasOneItemPerChoice() throws {
        ...
        XCTAssertEqual(columns.items.count, AppSettings.ColumnLayout.allCases.count)

        for (item, layout) in zip(columns.items, AppSettings.ColumnLayout.allCases) {
    ...
    func testAChoiceRoundTripsThroughTheDefaults() throws {
        ...
        for layout in AppSettings.ColumnLayout.allCases {
            ...
            XCTAssertEqual(AppSettings.ColumnLayout.allCases.filter {
